...
class UserServiceTest {
    ...
    void setUp() {
        ...
        ReflectionTestUtils.setField(userService, "isCaptchaEnabled", true);
        ...
        registerRequest.setCaptchaToken("valid_token");

        loginRequest = new LoginRequest();
        loginRequest.setEmail("test@example.com");
        loginRequest.setPassword("password123");
        loginRequest.setCaptchaToken("valid_token");
    ...
    void registerUserAccount_ShouldRegisterSuccessfullyAccount() {
        ...
        when(captchaUtils.verifyCaptcha(registerRequest.getCaptchaToken())).thenReturn(true);
    ...
    @Test
    void registerUserAccount_ShouldThrowException_WhenCaptchaTokenIsInvalid() {
        // Arrange
        registerRequest.setCaptchaToken("invalid_token");

        when(userRepository.existsByEmail(registerRequest.getEmail())).thenReturn(false);
        when(captchaUtils.verifyCaptcha(registerRequest.getCaptchaToken())).thenReturn(false);
    ...
    @Test
    void authenticateUser_ShouldThrowException_WhenCaptchaTokenIsInvalid() {
        // Arrange
        loginRequest.setCaptchaToken("invalid_token");

        when(userRepository.findByEmail("test@example.com")).thenReturn(user);
        when(captchaUtils.verifyCaptcha(loginRequest.getCaptchaToken())).thenReturn(false);
